...
// Copyright (c) 2011-2012 betacoinclassic Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("betacoinclassic-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #betacoinclassicTEST0\r");
            Send(hSocket, "WHO #betacoinclassicTEST0\r");
        } else {
            // randomly join #betacoinclassic00-#betacoinclassic99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #betacoinclassic%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #betacoinclassic%02d\r", channel_number).c_str());
        }
